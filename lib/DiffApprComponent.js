...
var BaseDiffApprComponent = (function (_super){
  ...
  BaseDiffApprComponent.prototype.postApprove = function (status, err){
    ...
    if (status === Status.Success){
      ...
      var msg = core_1.message(r, 'msg_approve_version_error', 'error');
  ...
  BaseDiffApprComponent.prototype.handleError = function (err){
    ...
    if (err){
      if (err.status && !isNaN(err.status)){
        msg = core_1.messageByHttpStatus(err.status, r);
      }
    }
...
var DiffApprComponent = (function (_super){
  ...
  function DiffApprComponent(props, service, resourceService, getLocale, showMessage, showError, loading){
    var _this = _super.call(this, props, service.keys(), resourceService, showMessage, showError, loading) || this;
