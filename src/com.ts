...
export function Search(p: Props) {
  ...
  return (
    <label className={p.className}>
      ...
      {p.clear && <button type='button' id='clearQBtn' name='clearQBtn' hidden={!p.value} className='btn-remove-text' onClick={p.clear}/>}
      {p.toggle && <button type='button' id='toggleSearchBtn' name='toggleSearchBtn' className='btn-filter' onClick={p.toggle} />}
      {p.search && <button type='submit' id='searchBtn' name='searchBtn' className='btn-search' onClick={p.search} />}
    </label>
  );
  */
}
